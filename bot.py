...
try:
    ...
    print("ERRO CRÍTICO: O arquivo 'config.json' não foi encontrado na mesma pasta do bot.")
    exit()
...
def criar_imagem_anuncio(imagem_produto_url, preco, caminho_fundo="fundo.jpg", caminho_logo="logo.png", caminho_fonte="font.ttf"):
    try:
        ...
        draw.rectangle(
            (pos_caixa_x - 20, pos_caixa_y - 15, pos_caixa_x + largura_texto + 20, pos_caixa_y + altura_texto + 20),
            fill=(0, 0, 0, 128)
        )
        ...
        print(f"ERRO DE ARQUIVO: {e}. Verifique se 'fundo.jpg', 'logo.png' e 'font.ttf' estão na pasta.")
        return "ERRO_ARQUIVO"
...
def criar_post_afiliado(message):
    ...
    if dados_produto:
        ...
        if imagem_final == "ERRO_ARQUIVO":
            bot.reply_to(message, "❌ *ERRO:* Não encontrei os arquivos de imagem/fonte (`fundo.jpg`, `logo.png`, `font.ttf`). Certifique-se de que eles estão no repositório junto com o bot.")
            return
        if imagem_final:
            ...
            bot.reply_to(message, "❌ Desculpe, não consegui gerar a imagem personalizada. Verifique os logs do bot.")
    else:
        bot.delete_message(chat_id=msg_espera.chat.id, message_id=msg_espera.message_id)
...
print("Bot Designer de Afiliado iniciado!")
